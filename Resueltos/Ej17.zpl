...
param Eps := 0.000000000000000001;
...
subto Rel_YconX: forall <i,j> in I*J : Y[i,j] <= X[i,j] * L[i] * 1000000 ; 


subto MinimoIA: forall <j> in J: IA[j] >= (sum <i> in I : Y[i,j] * 2 / (3*1000000) ) ; 
...
subto CA_EslacombLineal : forall <j> in J: CA[j] == V[2,j] * 2 + V[3,j] * 7 + V[4,j] * 13 + V[5,j] * 27 + V[6,j] * 32 ; 

subto seg_2_izq : forall <j> in J : V[3,j] - Eps >= 0 ;
subto seg_3_izq : forall <j> in J : V[5,j] - Eps >= 0 ;
